namespace LineDetection.MathImageProcessing
{
    public static class GaussianBlurGrayscale
    {
        ...
        public static void Apply(YUVImage? image)
        {
            ...
            originalImage = new int[width * height];

            for (int i = 0; i < width * height; i++)
            {
                originalImage[i] = image.Bytes[i];
            }

            int[] newImage = new int[width * height];

            GaussBlur_4(originalImage, newImage);
        ...
        /// GaussBlur_4
        /// </summary>
        private static void GaussBlur_4(int[] source, int[] dest)
        {
            int[] bxs = BoxesForGauss(sigma, radius);
        ...
        /// BoxesForGauss
        /// </summary>
        private static int[] BoxesForGauss(float sigma, int radius)
        {
            var wIdeal = Math.Sqrt((12 * sigma * sigma / radius) + 1);
            ...
            var mIdeal = (double)(12 * sigma * sigma - radius * wl * wl - 4 * radius * wl - 3 * radius) / (-4 * wl - 4);
